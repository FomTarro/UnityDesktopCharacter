...
namespace UnityDesktopCharacter {
	public class LoggingManager : Singleton<LoggingManager> {
		...
		public bool DEBUG_LOG_DISPLAY { get; private set; }
		[Header("Debug Settings")]
		[SerializeField]
		private TMP_Text _debugDisplay = null;
		...
		public void SetDebugDisplay(bool enabled) {
			this.DEBUG_LOG_DISPLAY = enabled;
			this._debugDisplay.gameObject.SetActive(enabled);
		}

		private void SetDebugText(string text) {
			this._debugDisplay.text = text;
		}
		...
		private static void EnqueueLog(string logString, string stackTrace, LogType type) {
			...
			if (INITIALIZED && Instance != null && Instance.DEBUG_LOG_DISPLAY) {
				Instance.SetDebugText(record.ToString());
			}
		}
